...
class _GenerateExcelState extends State<GenerateExcel> {
  ...
  Future<void> generateExcel() async {
    ...
    for (var user in users) {
      if (!usersByClass.containsKey(user.className)) {
        usersByClass[user.className] = [];
      }
      usersByClass[user.className]!.add(user);
    }

    // Debug: Print grouped users by class
    usersByClass.forEach((className, users) {
      print('Class: $className, Students: ${users.length}');
    });
    ...
    for (var entry in usersByClass.entries) {
      ...
      print('Creating Excel for Class: $className with ${classUsers.length} students');
